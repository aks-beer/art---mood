...
// Удаляет CJK и прочие нерусские/нелатинские иероглифы, которые иногда
// просачиваются в ответ LLM (например "не直接"). Кириллица и латиница сохраняются.
...
B) ЭМОДЗИ-ОБЪЕКТЫ -> НЕ ищи настроение. Возьми СУЩЕСТВИТЕЛЬНЫЕ-предметы, которые этот эмодзи
   ...
   ❄️ -> зима, снег, лёд, мороз, метель (зимний пейзаж); 🍂 -> осень, листопад; ☁️ -> облака, небо.
ВАЖНО: для эмодзи-объекта картина должна БУКВАЛЬНО изображать этот предмет/сезон (❄️ = именно зимняя/снежная сцена, не любой сельский пейзаж).
...
0. "fits" — true/false: ДЕЙСТВИТЕЛЬНО ли сюжет/тема картины соответствует настроению пользователя? Будь честен и строг. Например, для снежинки ❄️ подходит зимняя/снежная сцена, а сельский двор с курами — НЕ подходит (fits=false). Если связь натянутая или её нет — ставь false: такую картину мы НЕ покажем.

1. "why_fits" — 1-2 предложения: ЖИВАЯ связь темы картины с настроением (только для fits=true). КАТЕГОРИЧЕСКИ запрещено писать оговорки о несоответствии ("не имеет прямого отношения", "может не перекликаться", "не напрямую"). Если тянет на такую оговорку — значит fits=false.
  ...
  "descriptions": [
    {
      ...
      const descriptions = extractJSON(descResult.choices[0].message.content).descriptions as { index: number; fits?: boolean; why_fits: string; about: string }[];
      ...
      for (let i = 0; i < foundPaintings.length && selectedCards.length < DISPLAY; i++) {
        ...
        const whyFits = stripForeignGlyphs(desc?.why_fits || '');
        // Гейт релевантности: модель сама пометила fits=false ИЛИ текст содержит оговорку.
        if (desc && (desc.fits === false || mismatchRe.test(whyFits))) continue;
        ...
        selectedCards.push({
          ...
          year: p.year,
          image: p.imageUrl,
          why_fits: whyFits || "Картина подобрана ИИ-куратором под ваше настроение.",
          about: stripForeignGlyphs(desc?.about || '') || "Произведение из коллекции мирового музея, подобранное под ваше настроение.",
